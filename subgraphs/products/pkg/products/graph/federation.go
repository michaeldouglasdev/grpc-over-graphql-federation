...
func (ec *executionContext) resolveEntity(
	...
	case "FeaturedProductSection":
		resolverName, err := entityResolverNameForFeaturedProductSection(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "FeaturedProductSection": %w`, err)
		}
		switch resolverName {
		...
		case "findFeaturedProductSectionByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findFeaturedProductSectionByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindFeaturedProductSectionByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "FeaturedProductSection": %w`, err)
			}
...
func entityResolverNameForFeaturedProductSection(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for FeaturedProductSection", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for FeaturedProductSection", ErrTypeNotFound))
			break
		}
		return "findFeaturedProductSectionByID", nil
	}
	return "", fmt.Errorf("%w for FeaturedProductSection due to %v", ErrTypeNotFound,
